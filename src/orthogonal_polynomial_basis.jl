...
struct LegendrePolyBasis{T,A} <: OrthogonalPolyBasis
    ...
    function LegendrePolyBasis(n;wType=Float64)
        ...
        for i in 3:n
            ...
            one_below = ((2 * real_index - 1) / real_index) .* [[zero(wType)]; weights_helper[i-1,:][1:n-1]]
            weights_helper[i,:] .= one_below .- two_below
        end
        weights = [Tuple(weights_helper[i,j] for j in 1:i) for i in 1:n]
...
struct HermitePolyBasis{T,A} <: OrthogonalPolyBasis
    ...
    function HermitePolyBasis(n;wType=Int64)
        ...
        for i in 3:n
            ...
            for j in 2:i-1
                weights_helper[i,j] = 2 * weights_helper[i-1,j-1] - j * weights_helper[i-1,j+1]
            end
