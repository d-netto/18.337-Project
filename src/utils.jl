function compute_cov_matrix!(s::StochGalerkinODE,c::StochGalerkinODECache,matrix_cache,vector_transpose_cache,dim_,variance_map,u)
...
function compute_diag_variance(s::StochGalerkinODE,interval_t,sol_u)
    ...
    for index in indices_polys
        expectation_map[index] = mapreduce(i -> dot(quads[i].weights, (x -> poly_basis[i](x,index[i] - 1)).(quads[i].nodes)),*,Base.OneTo(length(index))) / norm_const
        # Hack for handling very small (abs val < 10^-10) and negative variances
        variance_map[index] = abs(normalization_map[index] / norm_const - expectation_map[index]^2)
    end
    ...
    for i in Base.OneTo(length(interval_t))
        push!(expectations, mapreduce(index -> (@view sol_u[i][:,index]) .* expectation_map[index],+,indices_polys))
...
function compute_expected_variance_exluding_one_variable!(s::StochGalerkinODE,variance_quad_index_cache,variance_index_cache,poly_prod,u,t,variable_index)
    ...
    sz_others = Tuple(num_polys[i] for i in Base.OneTo(length(num_polys)) if i != variable_index)
    ...
    norm_const_this_var = mapreduce(j -> quads[variable_index].weights[j],+,Base.OneTo(num_polys[variable_index]))
    for j in Base.OneTo(num_polys[variable_index])
        variance_this_var = dot(quads[variable_index].weights,(x -> poly_basis[variable_index](x,j - 1)^2).(quads[variable_index].nodes)) / norm_const_this_var -
                   (dot(quads[variable_index].weights,(x -> poly_basis[variable_index](x,j - 1)).(quads[variable_index].nodes)) / norm_const_this_var)^2
    ...
    join_index = (i,index) -> CartesianIndex(Tuple(j == variable_index ? i : index[effective_index(j)] for j in Base.OneTo(length(index) + 1)))
    ...
    for quad_index in CartesianIndices(sz_others)
        ...
        prod_weights = mapreduce(j -> j == variable_index ? one(uType) : quads[j].weights[quad_index[effective_index(j)]],*,Base.OneTo(num_vars))
        ...
        for index in CartesianIndices(sz_others)
            poly_prod[index] = mapreduce(j -> j == variable_index ? one(uType) : poly_basis[j](quads[j].
                                nodes[quad_index[effective_index(j)]],index[effective_index(j)] - 1),*,Base.OneTo(num_vars))
        end
        ...
        for j in Base.OneTo(num_polys[variable_index])
            ...
            variance_quad_index_cache .+= variance_index_cache.^2 .* (variance_this_poly_family_map[j])
        end
...
function compute_total_order_sobol_indices(s::StochGalerkinODE,sol,interval_t,variable_index)
    sol_u = [sol(t) for t in interval_t]
    c = StochGalerkinODECache(s,first(sol_u))
