...
struct StochGalerkinODE{
    ...
    function StochGalerkinODE(
        ...
        for index in CartesianIndices(normalization_map)
            normalization_map[index] = mapreduce(
                i -> dot(
                    grid.quads[i].weights,
        ...
        return new{
            ...
            typeof(vars),
            typeof(num_polys),
            typeof(indices_polys),
            typeof(poly_basis),
            typeof(grid),
            typeof(normalization_map),
        }(
            ode_func!,
            dim_,
            vars,
...
@inline function compute_poly_prod(
    s::StochGalerkinODE,
...
function stoch_galerkin_ode_dynamics!(
    s::StochGalerkinODE,
    c::StochGalerkinODECache,
    du,
    u,
